...
const ExtraIngredients = {
  MUSHROOMS: "Higienização Capilar",
  ONION: "Manicure (só mão)",
  PEPPER: "Escova",
  PINAPPLE: "Brow Lamination",
  TUNA: "Sobrancelha de Henna",
  MEAT: "Botox Capilar",
  CHEESE: "Progressiva a Laser",
  HOTSAUCE: "Botox Capilar",
  CORN: "Massagem Modeladora"
}
...
const ServiceDetails = () => {
  ...
  useEffect(() => {
    const existingService = cartProducts.find(item => item.id === id);
    if (existingService) {
      setExtraIngredients(existingService.extraIngredients);
    } else {
      setExtraIngredients([]);
    }
  }, [cartProducts, id]);
            ...
            <Col lg="6" md="6">
              <div className="single__product-content">
                ...
                <button onClick={addItem} className="addTOCART__btn">
                  {cartProducts.find(item => item.id === id) ? 'Update Cart' : 'Add to Booking'}
                </button>
              </div>
            </Col>
